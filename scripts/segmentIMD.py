...
input_path = "/Users/azhark/iCloud/dev/SVMatrixGenerator/data/PCAWG/SV/segments/samples/" #path to bedpe files
output_path = "/Users/khandekara2/iCloud/dev/SVMatrixGenerator/data/PCAWG/SV/segments/" #path to segment files
...
for file in os.listdir('.'):
    if file.endswith('.bedpe.tsv') and file in high_confidence:
...
with open("/Users/azhark/Documents/dev/HRD-Signatures/data/chr_sizes.txt") as f:
    for line in f:
        line = line.strip()
        (key, val) = line.split('\t')
        ranges[str(key)] = val
